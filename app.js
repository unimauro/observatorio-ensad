...
const GRANATE = '#7d2ba6', GRANATE2 = '#5a1d7a', ORO = '#d4a537', TEAL = '#0e7c86';
...
async function boot() {
  DATA.presu = await load('presupuesto-ensad.json');
  DATA.prov = await load('proveedores-ensad.json');
  DATA.plan = await load('planilla-ensad.json');
  render();
}
...
function render() {
  renderKpis(); renderPresu(); renderGasto(); renderProv(); renderPlan();
}
...
function renderKpis() {
  ...
  if (DATA.plan?.personas?.length) k.push(['Personal', fmtN(DATA.plan._meta?.total || DATA.plan.personas.length), 'docentes/funcionarios/admin.']);
...
function renderPresu() {
  ...
  new Chart(cEjec, {
    type: 'line',
    data: { labels: closed.map(x => x.year), datasets: [{ label: '% ejecución (años cerrados)', data: closed.map(x => x.ejec_pct), borderColor: GRANATE, backgroundColor: 'rgba(125,43,166,.12)', fill: true, tension: .3, spanGaps: true }] },
...
function renderGasto() {
  ...
  if (d.por_categoria?.length) donut(cUni, d.por_categoria);
...
function renderProv() {
  ...
  if (!V?.proveedores?.length) {
    w.innerHTML = `<div class="soon"><h3>🔎 En construcción</h3><p style="margin:0;color:var(--muted)">Descargando y cruzando las contrataciones de la ENSAD (OECE/CONOSCE) para listar cada proveedor, su RUC, el monto y sus dueños/representantes.</p></div>`;
    return;
  }
  const t = V.totales || {};
  // KPIs
  ...
  w.innerHTML = `<div class="card">
    <h3>Proveedores de la ENSAD · ${fmtN(V.proveedores.length)}</h3>
...
function renderPlan() {
  ...
  if (kp) kp.innerHTML = [
    ...
    ['Remun. máx', 'S/ ' + fmtN(Math.round(remun.max || 0)), esc((maxP?.cargo || '').slice(0, 24))],
...
function buildCtx() {
  const s = DATA.presu?.serie;
  let c = 'Eres el asistente del Observatorio ENSAD, un portal ciudadano de transparencia de la Escuela Nacional Superior de Arte Dramático "Guillermo Ugarte Chamorro" (UE 123 del MINEDU), con datos públicos (MEF, OECE). Responde corto, en español, solo sobre ENSAD y sus datos. No inventes cifras.';
  ...
  if (DATA.prov?.totales) c += ` Contrataciones: S/${(DATA.prov.totales.monto_total / 1e6).toFixed(1)}M a ${DATA.prov.totales.n_proveedores} proveedores.`;
  return c;
}
